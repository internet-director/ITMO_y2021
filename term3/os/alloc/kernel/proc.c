...
void dump(void) {
  struct proc *p = myproc();
  // acquire(&p->lock);
  for (uint64 *i = &p->trapframe->s2, j = 2; i <= &p->trapframe->s11;
       i++, j++) {
    printf("s%d: %d\n", j, *(uint32 *)i);
  }
  // release(&p->lock);
...
int dump2(int pid, int register_num, uint64 *result_value) {
  ...
  for (pp = proc; pp < &proc[NPROC]; pp++) {
    ...
    if (pid == pp->pid) {
      int code = 0;
      if (p->pid != pp->pid && pp->parent->pid != p->pid) {
        code = -1;
        goto end;
      }

      uint64 *ptr = &pp->trapframe->s2 + register_num - 2;
      if (copyout(p->pagetable, (uint64)result_value, (char *)ptr, 8) < 0)
        code = -4;
