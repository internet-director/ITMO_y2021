...
int cow_catch(uint64 *pagetable, int i) {
  pte_t *pte;
  uint64 pa;
  char *mem;

  if (i >= MAXVA || i <= 0) return -1;

  if ((pte = walk(pagetable, i, 0)) == 0 || (*pte & PTE_V) == 0 ||
      (*pte & PTE_U) == 0)
    return -1;
  ...
  if ((*pte & PTE_RSW) != 0) {
    ...
    *pte = (PA2PTE(mem) | PTE_FLAGS(*pte)) & ~PTE_RSW;
  }
  kfree((char *)pa);
  return 0;
}
...
void usertrap(void) {
  ...
  if (err_code == 8) {
    ...
    if (cow_catch(p->pagetable, r_stval()) != 0) p->killed = 1;
